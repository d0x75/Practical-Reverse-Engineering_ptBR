...
Ao operar em Protect-mode, a arquitetura x86 fornece 8 registradores de uso geral de 32 bits (GPRs). São eles:
...
tratá-los como valores de 64 bits em alguns cenários. Exemplo: , a instrução [RDTSC](https://docs.microsoft.com/pt-br/cpp/
intrinsics/rdtsc?view=msvc-160) grava um valor de 64 bits em EDX:EAX



O registrador EFLAGS de 32 bits é usado para armazenar o status de operações aritméticas e outros estados de
execução. Quando a instrução anterior resultou em zero, **ZF**(Zero Flag) será definido como 1. As flags de EFLAGS são usados
principalmente para implementar ramificação condicionais.
