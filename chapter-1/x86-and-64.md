...
- E por fim o campo _DeferredContext_ define o com o último valor guardado em *EAX*.
...
O compilador decidiu juntar três instruções em uma, porque conhecia as constantes com antecedência e quer economizar espaço. A versão
...
Isso é melhor compreendido por meio de exemplos e prática:
		...
		; tem 4 bytes de comprimento (daí o fator de scale como = *4)
...
A linha 2 lê uma DWORD do offset EDI + 4 e então a usa como o Base address em um array na linha 3. Portanto, você sabe que o **EDI** é
provavelmente uma struct que tem um array no offset "+4". A linha 7 incrementa o Index. A linha 8 compara o Index com o offset do Base
Address da struct.

Com essas informações, esse pequeno loop pode ser descompilado da seguinte maneira:
...
**SCAS** compara implicitamente : _AL / AX / EAX_ com dados que começam no endereço de memória de EDI. EDI é decrementado/incrementado
automaticamente dependendo da **DF** flag. Devido à sua semântica, o **SCAS** é comumente usado junto com o prefixo REP para
encontrar um byte, word ou dword em um buffer. Por exemplo, a função C strlen () pode ser implementada da seguinte forma:
...
**STOS** é o mesmo que _SCAS_, exceto  que grava o valor de _AL / AX / EAX_  em EDI. Isso é constantemente usado para inicializar um buffer com um valor constante(como memset()). Aqui está um exemplo :
...
Observação : **LODS** é outra instrução da mesma família. Ele lê um valor de 1, 2 ou 4 bytes do ESI e o armazena em AL, AX ou EAX.
